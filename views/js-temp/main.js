...
var resizePizzas = function(size) {
  ...
  if (size > 3 || size < 1) console.log("bug in changeSliderLabel");
  document.querySelector("#pizzaSize").innerHTML = ["Small", "Medium", "Large"][size - 1];

  // Iterates through pizza elements on the page and changes their widths
function changePizzaSizes(size) {
  const pizzas = document.querySelectorAll(".randomPizzaContainer");
  const newWidthPercent = size === "1" ? 25 : size === "2" ? 33.33 : 50;

  for (let i = 0; i < pizzas.length; i++) {
    pizzas[i].style.width = newWidthPercent + "%";
  }
}
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const preload = document.querySelector('link[as="image"][href="images/pizza2.png"]');
  for (var i = 0; i < 200; i++) {
